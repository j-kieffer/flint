...
void
acb_theta_sum_jet_radius(arf_t R2, arf_t eps, const arb_mat_t cho,
    const arb_t rho, arb_srcptr v, slong ord, slong prec)
{
    ...
    acb_theta_sum_radius(R2, eps, cho, rho, 0, prec, 0);
    ...
    if (arf_cmp(cmp, R2) <= 0)
    {
        acb_theta_sum_radius(R2, eps, cho, rho, ord, prec, 0);
        arb_div(t, nx, na, lp);
